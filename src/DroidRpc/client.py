        ...
        for batch in input_matrix:
            try:
                message = bot_pb2.BatchHedge(
                    ...
                    bid_price=array_to_bytes(batch[5].astype(float)),
                    margin=array_to_bytes(batch[6].astype(float)),
                    last_hedge_delta=array_to_bytes(batch[7].astype(float)),
                    last_share_num=array_to_bytes(batch[8].astype(float)),
                    total_bot_share_num=array_to_bytes(batch[9].astype(float)),
                    bot_cash_balance=array_to_bytes(batch[10].astype(float)),
                    expire_date=array_to_bytes(batch[11].astype(np.datetime64)),
                    price_level_1=array_to_bytes(batch[12].astype(float)),
                    price_level_2=array_to_bytes(batch[13].astype(float)),
                )
            except TypeError as e:
                print(e)
            except Exception as e:
